...
//        Page<Chart> chartPage = chartService.page(new Page<>(current, size));
    ...
    public BaseResponse<BiResponse> genChart(@RequestPart("file") MultipartFile multipartFile,
        ...
        User loginUser = userService.getLoginUser(request);
        // 限流判断，每个用户一个限流器
        redissonManager.doLimiter("genChartByAi_" + loginUser.getId());
    ...
    public BaseResponse<BiResponse> genChartByAsync(@RequestPart("file") MultipartFile multipartFile,
        ...
        redissonManager.doLimiter("genChartByAi_" + userId);
        ...
        String message =stringBuilder.append("原始数据").append("\n").append(result).toString();
        ...
        boolean result1 = chartService.save(chart);
        ThrowUtils.throwIf(!result1,ErrorCode.PARAMS_ERROR,"保存图表数据错误");
        ...
        CompletableFuture.runAsync(() -> {
            ...
            String strData = aiManager.doChart(message);
            ...
            boolean resultBoolean = chartService.updateById(updateResultChart);
            ThrowUtils.throwIf(!resultBoolean,ErrorCode.SYSTEM_ERROR);
        }, threadPoolExecutor);
    ...
    public BaseResponse<BiResponse> genChartByAsyncMq(@RequestPart("file") MultipartFile multipartFile,
        ...
        Long sendId = chart.getId();

        messageProducer.sendMessage(String.valueOf(sendId));

        BiResponse biResponse = new BiResponse();
        biResponse.setChartId(sendId);
        return ResultUtils.success(biResponse);
    }
